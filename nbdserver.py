...
def serveclient():
    ...
    afile.seek(0, 2)
    asock.send('NBDMAGIC\x00\x00\x42\x02\x81\x86\x12\x53' +
               struct.pack('>Q', afile.tell()) + '\0' * 128)
    while True:
        header = recvall(asock, struct.calcsize('>LL8sQL'))
        print("CALCSIZE: ", struct.calcsize('>LL8sQL'))
        print("RECEVED HEADER: ", header)
        magic, request, handle, offset, dlen = struct.unpack('>LL8sQL', header)
        print("MAGIC: ", magic, " REQUEST: ", request,  "HANDLE:",
              handle, "OFFSET:", offset, "DLEN:", dlen)
